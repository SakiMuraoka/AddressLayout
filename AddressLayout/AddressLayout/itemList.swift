...
struct itemList: View {
    ...
    var body: some View {
        List {
            ForEach(self.envelopeData.EnvelopeDataModel, id: \.id) {LogData in
                //                if (filter(text: self.filterData.searchText, LogData: LogData, filterData: self.filterData)) {
                NavigationLink(destination: PreviewView(currentEnvelopeData: self.currentEnvelopeData)) {
                    itemRow(LogData: LogData)
                        .onTapGesture {
                            self.currentEnvelopeData.envelope = LogData
                    }
                }
                //                }
            }
...
//
//
//func filter(text: String, LogData: EnvelopeData, filterData: filterViewModel) -> Bool{
//    var isHidden = true
...
struct itemList_Previews: PreviewProvider {
    static var previews: some View {
        itemList(filterData: filterViewModel(), envelopeData: logViewModel(), currentEnvelopeData: EnvelopeDataViewModel())
    }
}
